...
# CUSD/USDT
cleos push action relay.xy setreserve '[{"contract": "token.cusd", "symbol": "4,CUSD"}, {"contract": "token.usdt", "symbol": "4,USDT"}]' -p relay.xy
cleos push action relay.xy setreserve '[{"contract": "token.usdt", "symbol": "4,USDT"}, {"contract": "token.cusd", "symbol": "4,CUSD"}]' -p relay.xy

# Actions
cleos -v transfer -f bob relay.xy "1.0000 XY" "EOS" --contract token.xy
cleos -v transfer bob relay.xy "1.0000 EOS" "XY"

cleos -v transfer bob relay.xy "1.0000 USDT" "CUSD" --contract token.usdt
cleos -v transfer -f bob relay.xy "1.0000 CUSD" "USDT" --contract token.cusd
